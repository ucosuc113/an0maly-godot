...
func _process(delta: float) -> void:
	...
	var bh := black_hole
	var sphere := clampf(0.055 * (1.0 + 0.12 * log(1.0 + mass)), 0.04, 0.13)
	bh.sphere_radius = lerpf(bh.sphere_radius, sphere, k)
	var disc := 0.3 + 0.15 * sat + 0.03 * sin(_time * 7.0) * inst
	bh.disc_radius = lerpf(bh.disc_radius, disc, k)
	bh.disc_opacity = lerpf(bh.disc_opacity, 0.3 + 0.2 * sat, k)
	bh.bend_strength = lerpf(bh.bend_strength, 1.3 + 0.8 * clampf(mass / 50.0, 0.0, 1.0), k)
	...
	bh.disc_brightness = lerpf(bh.disc_brightness, (1.4 + 1.6 * heat + inst * 1.5) * flicker, k)

	bh.doppler_strength = lerpf(bh.doppler_strength, 0.2 + 0.6 * spin, k)
	bh.swirl_speed = lerpf(bh.swirl_speed, 0.7 + sim.disc_spin / 20000.0, k)
	bh.turbulence = lerpf(bh.turbulence, 0.6 + 0.4 * inst, k)
	bh.filament_contrast = lerpf(bh.filament_contrast, 0.5 + 2.5 * inst, k)

	var wobble: float = sim.core_misalign / 40.0 + inst * 6.0
	bh.disc_tilt_degrees = _base_tilt + Vector3(sin(_time * 1.3), 0.0, cos(_time * 0.9)) * wobble
